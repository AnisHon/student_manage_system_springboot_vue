<template>
  <div class="student-scores" style="height: 100%; padding: 20px;">
    ...
    <div style="height: 100%" ref="scrollRef">
      ...
      <el-table :data="classData" min-height="100px" ref="tableRef"
    ...
    <el-dialog v-model="addSubjectDialogVisible" title="班级管理" width="500">
...
function load() {
  get('/teacher/classes/' + (page.value + 1) + "?search=" + search.value,
      (data) => {
        if(data.length !== 0) {
...
function submit() {
  let url = changeOrAdd.value ?  "/admin/change-class" : "/admin/add-class"
...
function changeScore(index) {
  ...
  subjectForm.id = classData[index].id
  subjectForm.name = classData[index].name
...
function getData(search) {
  ...
  for (let i = 1; i <= page.value; i++) {
    get('/teacher/classes/' + i + "?search=" + search,
        (data) => {
...
function deleteSelection() {
  ...
  deleteList.forEach(element => {
    let index = classData.indexOf(element)
    if (index !== -1) {
      classData.splice(index, 1);
      get("/admin/delete-class/" + element.id, () => {
        ElMessage.success("删除了班级：" + element.name)
      })
    }
  })
}
